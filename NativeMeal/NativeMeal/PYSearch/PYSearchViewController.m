...
@interface PYSearchViewController () <UISearchBarDelegate, UITableViewDelegate, UITableViewDataSource,UIGestureRecognizerDelegate>
/** 头部内容view */
...
//
////@property(nonatomic,weak)StaticSegmentControl * staticSegmentControl;
@property(nonatomic, weak)SearchResultViewController * searchResultVC;
...
-(SearchResultViewController *)searchResultVC{
    if (!_searchResultVC) {
        ...
        searchResultVC.view.frame = CGRectMake(0, 0, self.view.py_width, self.view.py_height);
        searchResultVC.view.backgroundColor = self.baseSearchTableView.backgroundColor;
...
//        PYSearch.bundle/empty
        [emptyButton setImage:[UIImage imageNamed:@"search_for_clear_history_icon"] forState:UIControlStateNormal];
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(tapHistoryLabelChangeSearchBarText:) name:@"tapHistoryLabelChangeSearchBarText" object:nil];
    [self cancelSideBack];
}
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"endEdit" object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"tapHistoryLabelChangeSearchBarText" object:nil];
    [self startSideBack];
}
...
- (void)setup
{
    ...
    [self.navigationItem.rightBarButtonItem setTintColor:[UIColor colorWithRed:0.671 green:0.675 blue:0.714 alpha:1.00]];
    [self.navigationItem.rightBarButtonItem setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:17.0f],NSFontAttributeName, nil] forState:UIControlStateNormal];
...
- (void)setSearchHistoryStyle:(PYSearchHistoryStyle)searchHistoryStyle
{
    ...
    switch (searchHistoryStyle) {
        ...
        case PYSearchHistoryStyleBorderTag: // 边框标签
            for (UILabel *tag in self.searchHistoryTags) {
                // 设置背景色为clearColor
                tag.backgroundColor = [UIColor colorWithRed:0.961 green:0.973 blue:0.980 alpha:1.00];
                // 设置边框颜色
...
- (UILabel *)labelWithTitle:(NSString *)title
{
    ...
    label.text = title;
    label.textColor = [UIColor colorWithRed:0.149 green:0.157 blue:0.220 alpha:1.00];
//f5f8fa
    label.backgroundColor = [UIColor py_colorWithHexString:@"f5f8fa"];
...
-(void)addHistoryText:(NSString *)text{
    // 先移除再刷新
    [self.searchHistories removeObject:text];
    [self.searchHistories insertObject:text atIndex:0];
    // 保存搜索信息
    [NSKeyedArchiver archiveRootObject:self.searchHistories toFile:self.searchHistoriesCachePath];
    if (self.searchHistoryStyle == PYSearchHistoryStyleCell) {
        // 刷新cell
...
//    self.staticSegmentControl.view.hidden = self.searchSuggestionHidden || !searchText.length;
//    self.staticSegmentControl.searchText = searchText;
//    [self.view bringSubviewToFront:self.staticSegmentControl.view];
...
- (void)tapHistoryLabelChangeSearchBarText:(NSNotification *)notification{
    ...
    // 如果代理实现了代理方法则调用代理方法
    if ([self.delegate respondsToSelector:@selector(searchViewController:searchTextDidChange:searchText:)]) {
        [self.delegate searchViewController:self searchTextDidChange:self.searchBar searchText:searchText];
    }
    self.searchResultVC.searchText = searchText;
}
@end
