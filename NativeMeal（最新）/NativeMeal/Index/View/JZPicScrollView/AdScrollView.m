...
- (void)setImageNameArray:(NSArray *)imageNameArray
{
    ...
    _isTimeUp = NO;
    UIImage * image = [self buttonImageFromColor:[UIColor colorWithRed:0.933 green:0.933 blue:0.933 alpha:1.00]];
    _imageNameArray = imageNameArray;

    [_leftImageView setImageWithURL:[NSURL URLWithString:_imageNameArray[0]] placeholderImage:image];
    [_centerImageView setImageWithURL:[NSURL URLWithString:_imageNameArray[1]] placeholderImage:image];
    [_rightImageView setImageWithURL:[NSURL URLWithString:_imageNameArray[2]] placeholderImage:image];
}
...
- (void)addPageControl
{
    if ([self superview].subviews.count > 2) {
        [[self superview].subviews[2] removeFromSuperview];
    }
...
- (void)animalMoveImage
{
    ...
    _isTimeUp = YES;
    _changeImageTime = [NSTimer scheduledTimerWithTimeInterval:0.4f target:self selector:@selector(scrollViewDidEndDecelerating:) userInfo:nil repeats:NO];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if (self.contentOffset.x == 0)
    {
        ...
        _pageControl.currentPage = (_pageControl.currentPage - 1+_imageNameArray.count)%_imageNameArray.count;
    }
...
//    UIImage * image = [self buttonImageFromColor:[UIColor colorWithRed:0.933 green:0.933 blue:0.933 alpha:1.00]];
    ...
    [_leftImageView setImageWithURL:[NSURL URLWithString:_imageNameArray[(currentImage-1+_imageNameArray.count)%_imageNameArray.count]] placeholderImage:image];
    _leftAdLabel.text = _adTitleArray[(currentImage-1+_imageNameArray.count)%_imageNameArray.count];
    
    [_centerImageView setImageWithURL:[NSURL URLWithString:_imageNameArray[(currentImage+_imageNameArray.count)%_imageNameArray.count]] placeholderImage:image];
    
    _centerAdLabel.text = _adTitleArray[(currentImage+_imageNameArray.count)%_imageNameArray.count];
    
    [_rightImageView setImageWithURL:[NSURL URLWithString:_imageNameArray[(currentImage+1+_imageNameArray.count)%_imageNameArray.count]] placeholderImage:image];
    _rightAdLabel.text = _adTitleArray[(currentImage+1+_imageNameArray.count)%_imageNameArray.count];
...
- (void)tapImageViewWithFocusImg:(UITapGestureRecognizer *)gesture{
    if (self.myTapCurrentImgBlock) {
        self.myTapCurrentImgBlock((currentImage-1+_imageNameArray.count)%_imageNameArray.count);
    }
}
//通过颜色来生成一个纯色图片
